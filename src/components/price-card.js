  ...
  return (
    <Card className={header ? 'package__card active' : 'package__card'} sx={styles.pricingBox}>
      ...
      <Box>
        ...
        <Text className='package__price'sx={styles.price}>{priceWithUnit} <span>/month</span></Text>
        <Box sx={styles.buttonGroup}>
          <Button variant='primary' aria-label={buttonText}>
            {buttonText}
          </Button>
          {anotherOption && (
            <Button variant='textButton' className="free__trial" aria-label={anotherOption} sx={{ color: 'red' }}>
              {anotherOption}
            </Button>
          )}
        </Box>
